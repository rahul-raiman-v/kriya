	...
	<div className='h-screen overflow-auto' ref={ref}>
		...
		{items.map((item,idx)=>{
			...
			<TimelineItem key={item.date} >
				...
				<TimelineContent>
				<div className={cn('border border-gray-300 p-4 bg-white rounded-md flex gap-3 mt-10 mb-5 shadow-lg backdrop-blur-lg cursor-pointer', isHovered === idx && "shadow-2xl backdrop-blur-xl border-blue-400 border-2")} 
					onMouseEnter={() => {
						setIsHovered(idx);
						setIsPaused(true)
						}} 
					...
					<p className={cn(' font-semibold text-gray-700 text-left line-clamp-2' , isHovered === idx && "line-clamp-none")}>{item?.description}</p>
					</div>
